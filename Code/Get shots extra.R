get_shots_extra <- function(file_path, name_detail, save_files = T){
  ...
  players_pos <- players %>%
    mutate(position = role$name) %>%
    dplyr::select(wyId, foot, position)
  ...
  shots_ok <- shots %>%
    ...
    filter(is_blocked == 0) %>%
    dplyr::select(-c(8:13)) %>%
    left_join(players_pos, by = c("playerId" = "wyId")) %>%
