...
```{r}
modelname2 <- "GBM_2_AutoML_7_20251215_152444" # change this name if you want to use another model saved by yourself
...
logitnorm_summary <- data.frame(
  ...
  Mean = round(LogitNorm_mix$parameters$mean, 4),
...
```{r}
cat("\n=== SUMMARY ===\n")
cat("Data range:", min(xg), "to", max(xg), "\n")
cat("Mean xG:", mean(xg), "\n")
cat("Median xG:", median(xg), "\n")
cat("SD xG:", sd(xg), "\n\n")
