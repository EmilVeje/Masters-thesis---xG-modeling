...
Setting testing dataset and features for right-footed players:
```{r}
h2o.init(nthreads = -1, max_mem_size = "4G")
...
```{r}
modelname2_left <- "" #Insert modelname for left-footed players here
cmod2_left <- h2o.loadModel(paste0("saved-models/", modelname2_left))
```

Loading right-footed model with all features:
```{r}
modelname2_right <- "" #Insert modelname for right-footed players here
...
ROC curve:
```{r}
# Convert H2O objects to R dataframes
